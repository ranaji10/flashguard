# The first half of this file was an older copy of the same test classes. Python kept only
# the second definition of each class, so 42 test methods never ran. Removed 24 September 2026.
import copy
...
class VerifyV2Test(unittest.TestCase):
    ...
    def test_locked_prerequisite_is_not_ready(self):
    ...
    def test_minimum_version_below_requirement_is_not_ready(self):
...
class VersionComparisonTest(unittest.TestCase):
    ...
    def test_exact_major_comparison(self):
        ...
        for mismatch_val in (13, "13", 11, "11", "13.0"):
            ...
            res = verify(fp, recipe)
            self.assertEqual(res["verdict"], "not-ready", f"exact_major mismatch on {mismatch_val} is not-ready")
            codes = [r["code"] for r in res["reasons"]]
    ...
    def test_minimum_comparison(self):
        ...
        self.assertEqual(res_fail["verdict"], "not-ready")
...
class SpacewarRealCaptureTest(unittest.TestCase):
    ...
    def test_real_capture_is_not_ready_on_bootloader_state_mismatch(self):
    ...
    def test_synthetic_unlocked_spacewar_wrong_android_version_is_not_ready(self):
        fp = self._load_real_spacewar_fingerprint()
        fp["bootloader_state"] = "unlocked"
...
class UpstreamUntestedGateTest(unittest.TestCase):
    ...
    def test_untested_recipe_on_locked_phone_is_not_ready(self):
        """not-ready > cannot-verify: an unmet prerequisite on a confirmed phone is a definite finding."""
        recipe = copy.deepcopy(BASE_RECIPE)
        recipe["source"]["upstream_untested"] = True
