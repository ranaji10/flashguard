...
if is_in "$VID" "$PHONE_VENDORS" && [ "$CLASS" != "adb" ] && [ "$CLASS" != "fastboot" ]; then
  cat <<'PHONE'
...
emit "BENCH_CAPTURE {\"device_class\":\"$(jesc "$CLASS")\",\"usb_vendor_id\":\"$VID\",\"usb_product_id\":\"$PID\",\"usb_interface_class\":\"$(jesc "$ICLASS")\",\"classifier_confidence\":$CONF,\"_lsusb\":\"$(jesc "$DESC")\",\"_product_string\":\"$(jesc "${IPROD:-}")\",\"_iface_string\":\"$(jesc "${IFACE:-}")\",\"_hint\":\"$(jesc "$HINT")\",\"_detected\":true}"
